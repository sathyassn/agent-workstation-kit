...
- Chromium/Xvfb and browser libraries on Linux; Google Chrome on macOS. Each project pins Playwright and installs its matching browser build.
...
Codex and Claude Code use their documented npm distributions through mise. Grok Build is installed only through xAI's reviewed macOS/Linux installer; the repository deliberately does not invent an npm package. Herdr uses its mise registry entry. Authentication remains a separate gate.
