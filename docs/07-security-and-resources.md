...
The supplied systemd policy governs the agent user's `user-UID.slice`; validate after reboot that the desktop, terminals, browsers, and tests descend from that slice. `CPUWeight=90` is a contention preference, not a core reservation. `os_cpu_reserve_threads` is capacity-planning headroom, not CPU pinning. The configured memory reserve sets `MemoryHigh`; half of it remains beyond the emergency `MemoryMax`, raised to 4 GiB when the configured reserve is larger than 4 GiB. `TasksMax` scales from 4,096 to 16,384 with RAM. Replace defaults with burn-in evidence. Preview rollback with `apply-resource-policy-linux.sh --agent ACCOUNT --remove`; apply it only with human approval, then end all target-user sessions or reboot.
